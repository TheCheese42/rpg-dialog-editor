...
func _on_new_button_pressed() -> void:
	if Globals.get_saved():
		...
		dialog.dialog_text = (
			"You still have unsaved changes.\n\n" +
			"Are you sure you want to create a new dialog?"
		)
		dialog.confirmed.connect(_new_dialog)
		await get_tree().create_timer(0.0).timeout
		add_child(dialog)
		dialog.popup_centered()


func _new_dialog() -> void:
	Globals.reset()
...
func _on_save_button_pressed() -> void:
	if not Globals.get_open_file():
		await _on_save_as_button_pressed()
	else:
		_save_file(Globals.get_open_file())
...
func _on_language_button_pressed() -> void:
	...
	get_tree().root.find_child("Popups", true, false).add_child(dialog)
